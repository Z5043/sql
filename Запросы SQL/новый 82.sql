...
(select
 ...
 from 
 KDW.W_SET_VALUES 
 where set_id=68733702), /*список регионов гаслп изменять под логином dev_scm*/ 
whs_spis as
(select 
j.terr_code reg,
 j.ID_WHSE, 
 ...
 from KDW.DWD_WHSE j,
	...
	from 
	KDW.W_SET_VALUES
	where set_id=70917967) k
	 where
	 j.terr_code='0'
	 and k.SET_VALUE=j.whse_code
			union all
		 select 
		 w.terr_code reg,
		 w.ID_WHSE,
...
FROM
  ...
  KDW.DWF_PO_L PO_L,
  KDW.DWD_PO_L_OTHER OTHER_PO_L,
  whs_spis ws, KDW.DWD_CALENDAR sd
WHERE
