...
vt_catalog_tovara_kdw_other_16 AS
	...
	WHERE
		...
		AND mvw.ITEM_NUM in ('1014319','1289221')
    )
,	
vt_catalog_tovara_kdw_other_27 as(
	...
	WHERE
		...
		AND rel_item.SUB_ITEM_NUM in ('1014319','1289221')
),
vt_table_full as
(
select 
...
ostatok_vt_table as (
	...
	WHERE
	  ...
	  AND ITEM_R.ITEM_NUM in ('1014319','1289221')
	GROUP BY
	  R_WHSE.WHSE_CODE,
	  ITEM_R.ITEM_NUM,
	  R_WHSE.TERR_CODE
...
mo_vt_table as (
	...
	WHERE
	  ...
	  AND zs.ITEM_NUM in ('1014319','1289221')
  )
  ,
priznak_zvp as (
	SELECT   
	...
	WHERE
	  ...
	  AND zvp.ITEM_NUM in ('1014319','1289221')
  )
,
priznak_vp as (
	SELECT   
	...
	AND vt_table_full.Код_склада = KDW.DWE_ORD_TRANSFER.WHSE_CODE
	  ...
	  AND KDW.DWE_ORD_TRANSFER.ITEM_NUM in ('1014319','1289221')
)
select 
vt_table_full.Артикул Артикул,
vt_catalog_tovara_kdw_other_16.Корневой_артикул,
