SELECT
  ...
  a.Кол_дней_Ср_ТЗ_02,
  a.Ср_ТЗ_в_наличии_м3_02,
  a.Ср_ТЗ_выделено_м3_02,
  a.Ср_ТЗ_свободно_м3_02,
  ...
  item_r_2 AS
    ...
    FROM
      KDW.DWF_ITEM_R ITEM_R JOIN vt_whse_code
                              ON ( ITEM_R.ID_WHSE = vt_whse_code.ID_Кода_склада )
    WHERE
     ( ITEM_R.ID_DATE BETWEEN ( SELECT kdw.getDateID((SELECT ADD_MONTHS(TRUNC(SYSDATE,'MM'),-2) FROM DUAL)) FROM dual )
                           AND ( SELECT kdw.getDateID((SELECT TRUNC(SYSDATE,'MM')-2 FROM DUAL)) FROM dual ) )
    )
  ,
    vt_tov_zap_02 AS
    (
    SELECT /*+ leading(vt_catalog_tovara_kdw_other_16) use_hash(vt_catalog_tovara_kdw_other_16 ITEM_R) */
      vt_catalog_tovara_kdw_other_16.Артикул,
      COUNT(DISTINCT ITEM_R_2.ID_DATE) Кол_дней_Ср_ТЗ_02,
      SUM(ITEM_R_2.ON_HAND * vt_catalog_tovara_kdw_other_16.Объём_см3 / (1000000)) / COUNT(DISTINCT ITEM_R_2.ID_DATE) Ср_ТЗ_в_наличии_м3_02,
      SUM(ITEM_R_2.COMMITTED_QTY * vt_catalog_tovara_kdw_other_16.Объём_см3 / (1000000)) / COUNT(DISTINCT ITEM_R_2.ID_DATE) Ср_ТЗ_выделено_м3_02,
      SUM(ITEM_R_2.ON_HAND * vt_catalog_tovara_kdw_other_16.Объём_см3 / (1000000)) / COUNT(DISTINCT ITEM_R_2.ID_DATE) - SUM(ITEM_R_2.COMMITTED_QTY * vt_catalog_tovara_kdw_other_16.Объём_см3 / (1000000)) / COUNT(DISTINCT ITEM_R_2.ID_DATE) Ср_ТЗ_свободно_м3_02
      
    FROM
      item_r_2 JOIN vt_catalog_tovara_kdw_other_16
               ON ( ITEM_R_2.ITEM_NUM = vt_catalog_tovara_kdw_other_16.Артикул )
    GROUP BY
      vt_catalog_tovara_kdw_other_16.Артикул
    HAVING
      SUM(ITEM_R_2.ON_HAND) > 0
    ),
    item_r_0 AS
    (
    SELECT /*+ leading(vt_whse_code) index(ITEM_R I7_ITEM_R_F) use_nl(vt_whse_code item_r) materialize */ *
...
,
 ...
 FROM
  ...
  SELECT
	...
	vt_tov_zap_02.Кол_дней_Ср_ТЗ_02,
    vt_tov_zap_02.Ср_ТЗ_в_наличии_м3_02,
    vt_tov_zap_02.Ср_ТЗ_выделено_м3_02,
    vt_tov_zap_02.Ср_ТЗ_свободно_м3_02,
  ...
  WHERE
	...
	AND ( vt_catalog_tovara_kdw_other_16.Артикул = vt_tov_zap_02.Артикул(+) )
...
sum(В_пути_на_приемкеМ2) В_пути_на_приемкеМ2,
...
a.spec_id,
 a.spec_date,
  ...
  (a.qty * b.VOLUME)/1000000 tvpM3,
   ...
   CASE WHEN a.lev=4  AND a.sts = '2'  then (a.qty * b.VOLUME)/1000000 else 0 end as В_пути_на_приемкеМ2,
