...
and (g_main_item_v.vend_whse_status='A')
   ...
   AND  
    R_CAL.CAL_DATE BETWEEN trunc(sysdate)-1
             AND trunc(sysdate)-1
...
CASE WHEN SUM(ITEM_R.tfer_in)<0 THEN 0 ELSE SUM(ITEM_R.tfer_in) end as tfer_in,
 ...
 WHERE
   ...
   AND R_CAL.CAL_DATE BETWEEN trunc(sysdate)-1
             AND trunc(sysdate)-1
   AND  R_ITEM.IND_CATEGORY  in ('О', 'D')
 ...
 ('Т100','Т32','Т35','Т33','Т61','Т62','Т63','Т64','Т65','Т66','Т67','Т68','Т69','Т70','Т44','Т45','Т46','Т50','Т51','Т54','Т55','Т56','Т57','Т58','Т59','Т78','Т79','Т80','Т81','Т82','Т83','Т84','Т85','Т86','Т87','Т91','Т92','Т93','Т90','Т94','Т95','Т96','Т97','Т98','Т99','Т70', 'Т109', 'Т110', 'Т111', 'Т112', 'Т113', 'Т114', 'Т115')
  ...
  WHERE
    ...
    AND ( KDW.DWE_OPEN_WHSE_T_L.OPEN_DATE BETWEEN trunc(sysdate)
                                              AND trunc(sysdate) )
    AND ( ow_ph.STATE = 1 )
...
(   SELECT   
   ...
   WHERE
     ...
     AND  R_WHSE. terr_code = '0' and r_whse.clp = 'Y' 
       AND s.id_date =(SELECT kdw.getZGLDateID(trunc(sysdate)-1) from dual)
        and s.N>5
   group by s.item_num
  ) ,
  a as
...
FROM
  KDW.DW_GOODS g
WHERE
  ( g.ITEM_NUM IN (SELECT SET_VALUE FROM KDW.W_SET_VALUES WHERE set_id IN ('122115231')))
UNION
SELECT
  g.ITEM_NUM  articul,
  '2_мебель'  list_attr
FROM
  KDW.DW_GOODS g
WHERE
  ( g.ITEM_NUM IN (SELECT SET_VALUE FROM KDW.W_SET_VALUES WHERE set_id IN ('53791673')))
),
top1 as
(
SELECT
...
FROM
  KDW.DW_GOODS g
WHERE
  ( g.ITEM_NUM IN (SELECT SET_VALUE FROM KDW.W_SET_VALUES WHERE set_id IN ('126841027') ))
),
dd
as
(
SELECT SUM(work_day) wd, yyyy_w FROM   KDW.DWD_CALENDAR
           WHERE CAL_DATE BETWEEN trunc(sysdate)-1
             AND trunc(sysdate)-1
group by yyyy_w
) 
...
SELECT item_num,ts, ts_name, sgrp_name, short_name, regcatalog, ind_category, m, f, v, dd.wd arto,
 ...
 vendor_num||' '||vendor_name vend, prod_manager,  prod_manager_name,
             ...
             BRAND_ID,  BRAND_NAME, TRADE_MARK_ID, TRADE_MARK_NAME,
            a.yyyy_w, bayer, top.list_attr, top1.list_attr1
 FROM a, dd, top, top1
where a.yyyy_w = dd.yyyy_w 
 and a.item_num = top.articul(+) 
 and a.item_num = top1.articul(+) 
...
m, f, v, dd.wd, dd.wd*f, dd.wd*v, vendor_num, vendor_name,
               ...
               BRAND_ID,  BRAND_NAME, TRADE_MARK_ID, TRADE_MARK_NAME, a.yyyy_w, bayer, top.list_attr, top1.list_attr1 
)
WHERE ts in ('Т35','Т33','Т61','Т62','Т63','Т64','Т65','Т66','Т67','Т68','Т69','Т70','Т44','Т45','Т46','Т50','Т51','Т54','Т55','Т56','Т57','Т58','Т59','Т78','Т79','Т80','Т81','Т82','Т83','Т84','Т85','Т86','Т87','Т91','Т92','Т93','Т90','Т94','Т95','Т96','Т97','Т98','Т99','Т70','Т32','Т100', 'Т109', 'Т110', 'Т111', 'Т112', 'Т113', 'Т114', 'Т115')