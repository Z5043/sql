select str_to_date(a.e_date,'%d.%m.%Y') as e_date, a.id, b.order_num, b.item_num, b.m_div, b.ordered_qty, b.deal_limit, c.ord2, c.item_num2, c.div2, c.order2, c.limit2, c.bayer_comment,
...
where 
 e_date BETWEEN str_to_date((@Prompt('1. Aaoa ia?aea ia?eiaa','A',,mono,free)),'%d.%m.%Y')   AND  str_to_date((@Prompt('2. Aaoa ieii?aiey ia?eiaa','A',,mono,free) ),'%d.%m.%Y')  
...
e_date BETWEEN str_to_date((@Prompt('1. Aaoa ia?aea ia?eiaa','A',,mono,free)),'%d.%m.%Y')   AND  str_to_date((@Prompt('2. Aaoa ieii?aiey ia?eiaa','A',,mono,free) ),'%d.%m.%Y')  
and segment is null
and orl_comment = 'Auaaeaii a iau?ia'
...
where
segment is null
and e_date BETWEEN str_to_date((@Prompt('1. Aaoa ia?aea ia?eiaa','A',,mono,free)),'%d.%m.%Y')   AND  str_to_date((@Prompt('2. Aaoa ieii?aiey ia?eiaa','A',,mono,free) ),'%d.%m.%Y')  
and orl_comment in ('Auaaeaii a iieiii iau?ia' , 'Auaaeaii a iau?ia')
...
/*segment is null*/
 e_date BETWEEN str_to_date((@Prompt('1. Aaoa ia?aea ia?eiaa','A',,mono,free)),'%d.%m.%Y')   AND  str_to_date((@Prompt('2. Aaoa ieii?aiey ia?eiaa','A',,mono,free) ),'%d.%m.%Y')  
and orl_comment in ('Auaaeaii a iieiii iau?ia' , 'Auaaeaii a iau?ia' , 'Auaaeaii a iaeneiaeuiii iau?ia, naiaiaiue inoaoie en?a?iai')
...
where
/*segment is not null*/
orl_comment in (ыв аыва 
'Coaoon caeaca a eniieuciaaiee',
'Coaoon caeaca caaa??ai',dfg ������ ��� ��� 
'Coaoon caeaca ia?eaoeiaiaue', ываыва 
'Coaoon caeaca ioiaiai',
'Iao oiaa?a ia ON',ыва ыва ываыва 
'Ionoonoaea aaiiie iiceoee a caeaca',
'No?iea iaania?aia ii o?aiceoiie oaoiieiaee',
'No?iea iaania?aia ?aiaa',
'No?iea ia?aaaia a ON'
)
and e_date BETWEEN str_to_date((@Prompt('1. Aaoa ia?aea ia?eiaa','A',,mono,free)),'%d.%m.%Y')   AND  str_to_date((@Prompt('2. Aaoa ieii?aiey ia?eiaa','A',,mono,free) ),'%d.%m.%Y')  
