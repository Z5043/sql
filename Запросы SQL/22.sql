 ...
 where j.terr_code='0' and k.SET_VALUE=j.whse_code),
  ...
  ,
    ...
    WHERE
      ...
      AND  (  mvw_goods.STATE  =  1 or mvw_goods.IND_CATEGORY <> 'H')
...
SELECT/*блок заданий на приход*/
	  ...
	  4 lev,
	  '' order_num,
	  PO_H.SPEC_NUM SPEC_NUM,
	  '' USER_CODE,
	  '' USER_NAME,
	  VENDOR_PO_H.VENDOR_NUM VENDOR_NUM,
	  VENDOR_PO_H.VEND_NAME VEND_NAME
FROM
	  ...
	  KDW.DWF_PO_H  PO_H,
	   KDW.DWD_U_OPER  USER_PO_H,
	  KDW.DWD_VENDOR  VENDOR_PO_H,
...
WHERE
	 ...
	 AND PO_H.PO_NUM = PO_L.PO_NUM
	 AND ( PO_H.ID_USER=USER_PO_H.ID_U_OPER  )
	 AND ( VENDOR_PO_H.ID_VENDOR=PO_H.ID_VENDOR  )
...
GROUP BY
	 ...
	 PO_H.SPEC_NUM,
	 VENDOR_PO_H.VENDOR_NUM,
	 VENDOR_PO_H.VEND_NAME
...
having
	...
	vt_table.Артикул,
	vt_table.qty Шт,
	vt_table.date_r Дата_поставки,
	vt_table.imp Импорт,
	vt_table.sts Статус,
	vt_table.lev,
	vt_table.spec_id id_Элита, 
	vt_table.spec_date Дата_спецификации,
	vt_table.Номер_заказа_САП,
	vt_table.Номер_специцикации,
	vt_table.Код_МЛ Код_автора_заказа,
	vt_table.МЛ Автор_заказа,
	vt_table.Код_поставщика Код_факт_поставщика,
	vt_table.Поставщик Факт_поставщик,
	
	logist.Код_МЛ,
    logist.Название_МЛ МЛ,
	logist.Код_поставщика,
    logist.Название_поставщика,
	
	vt_catalog_tovara_kdw_other_16.Объём_см3/1000000*vt_table.qty Кол_во_м3,
	vt_catalog_tovara_kdw_other_16.Признак_категория,
	
	TRUNC(SYSDATE - vt_table.date_r) Кол_дней,
	
	case 
		when TRUNC(SYSDATE - vt_table.date_r) < 10  then 'Менее 10 дней'
		when TRUNC(SYSDATE - vt_table.date_r) >= 10 and TRUNC(SYSDATE - vt_table.date_r) < 20   then 'От 10 до 20 дней'
		when TRUNC(SYSDATE - vt_table.date_r) >= 20 and TRUNC(SYSDATE - vt_table.date_r) < 40 then 'От 20 до 40 дней' 
		else 'Более 40 дней'
	end Группировка
	...
	WHERE 
		...
		AND vt_table.date_r < TRUNC(SYSDATE)