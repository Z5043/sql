...
WHERE
  ...
  AND  (
  ( r.id_reestr_date BETWEEN (SELECT kdw.getDateID(TO_DATE(@Prompt('1. Дата начала периода','A',,mono,free), 'DD.MM.YYYY')) FROM dual) AND (SELECT kdw.getDateID(TO_DATE(@Prompt('2. Дата окончания периода','A',,mono,free), 'DD.MM.YYYY')) FROM dual)  )
  AND  ( r_sv.DELIV = 2  )
  )
GROUP BY
  r_r_date.CAL_DATE, 
