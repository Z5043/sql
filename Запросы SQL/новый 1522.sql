...
e.LEAD_TIME6, e.STOCK_CONTROL6, e.b6, e.CROSS_DOCKING6, e.whse_code6, e.item_name6, e.unit6, e.m_msk6,
 i.ITEM_TS, i.SGRP_NAME, i.desc_1, i.desc_2, i.desc_3, i.ass_grp, i.div_name, t1.ccc2, t1.sdr, u.Количество

from 
(
select date_format(e_date,'%d.%m.%Y') as e_date,  ssc_id, item_num from ssc_l  
where 
...
left join 
(select div_name, item_num, ITEM_TS, SGRP_NAME, desc_1, desc_2, desc_3, ass_grp from item_ref ir )
as i on z.item_num=i.item_num
left join 
(SELECT
ir.item_num as Артикул,
ir.CCC2 as ccc2,
sws.sdr as sdr
from item_ref ir
Left Join shp_whs_sal_comp sws on ir.item_num=sws.item_num
where sws.region='0'
and ir.hsz='064'
and ir.ind_category in ('О','D')) t1 on z.item_num=t1.Артикул

left join
(
select item_num, count(item_num) as Количество from ssc_l 
where 
 e_date BETWEEN str_to_date((@Prompt('1. Дата начала периода','A',,mono,free)),'%d.%m.%Y')   AND  str_to_date((@Prompt('2. Дата окончания периода','A',,mono,free) ),'%d.%m.%Y')  
group by item_num 
) as u on z.item_num = u.item_num 