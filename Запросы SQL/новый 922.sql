...
WHERE
  ...
  AND  (
  ( (R_WHSE.WHSE_CODE IN @Prompt('7. Код склада','A',,multi,free) OR 'все' IN @Prompt('7. Код склада','A',,multi,free))  )
  ...
  AND  ( R_ITEM.ART_GL IN (select SET_VALUE from KDW.W_SET_VALUES where set_id  =@Prompt('22. Список товаров','A',,mono,free))  )
 AND  ( R_CAL.IS_CURRENT = 'Y'  )
  AND  R_ITEM.STATE  =  1
AND  R_ITEM.IND_CATEGORY  !=  'М'							   
  )
GROUP BY
  R_ITEM.ITEM_NUM, 
  R_ITEM.ITEM_NAME, 
